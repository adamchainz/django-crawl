...
No password is required, because django-crawl uses |Client.force_login()|__ to log in the user without checking credentials.

.. |Client.force_login()| replace:: ``Client.force_login()``
__ https://docs.djangoproject.com/en/stable/topics/testing/tools/#django.test.Client.force_login
...
Pass ``-c`` (``--command``) to run Python code for every response.
